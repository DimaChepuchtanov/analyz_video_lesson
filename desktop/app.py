...
from modules import (Auth as auth_module,
                     config)
from setting import (is_internet as is_inet,
                     user_id as uid,
                     reload_user as uid_reload,
                     schedule_internet as checker_inet)
...
class Main(Tkinter):
    def __init__(self, is_internet=False, screenName=None, baseName=None,
        ...
        if is_internet:
            ...
            self.is_internet_thread = threading.Thread(target=checker_inet,
                                                       name="IS_INTERNET")
        ...
        tree = CTable.CTkTable(master=s, row=6, height=30,
                               pady=1,
                               header_color="#ebe6f7",
                               colors=['white', 'white'],
                               corner_radius=10,
        ...
        self.get_mark_with_range({"start": datetime.datetime.strftime(datetime.datetime.now(), "%d.%m.%y"),
    ...
    def get_lessons(self, tag: str) -> list:
        ...
        if tag == 'day':
            ...
            result = [randint(0, 10) for x in range(day)]
    ...
    def exit_auth(self):
        ...
        config.remove_data_config(config_)
        self.destroy()
...
if __name__ == "__main__":
    ...
    if uid is None:
        auth_ = auth_module(is_internet=is_inet)
        auth_.mainloop()
        uid = uid_reload()
    ...
    main = Main(is_internet=is_inet, uid=uid)
    main.mainloop()
